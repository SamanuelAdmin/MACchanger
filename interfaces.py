...
def setInterfaceMac(iface, mac): # from https://github.com/feross/SpoofMAC/blob/master/spoofmac/interface.py#L110
    print('"', iface, '" -> ', mac)
    subprocess.call(f"ip link set {iface.name} down".split())
    subprocess.call(f"ip link set {iface.name} address {mac}".split())
    subprocess.call(f"ip link set {iface.name} up".split())
